...
class DataPreprocessingToolkit(object):
    ...
    def filter_out_low_prices(df):
        ...
        """
        ########################
        df = df[df['accommodation_price'] > 50]
        return df
        ########################
    ...
    def add_weekend_stay(df):
        ...
        """
        ########################
        df['weekend_stay'] = (((df['date_from'].dt.dayofweek >= 4) & (df['date_from'].dt.dayofweek != 6)) |
              (df['date_to'].dt.dayofweek >= 5) |
              ((df['date_to'].dt.dayofweek < df['date_from'].dt.dayofweek) & (df['date_from'].dt.dayofweek != 6)) |
              ((df['date_to'] - df['date_from']).dt.days >= 6))
    ...
    def aggregate_group_reservations(self, df):
        ...
        ########################
        agg_sum = group_reservations[['group_id'] + self.sum_columns].groupby('group_id').sum()
        agg_mean = group_reservations[['group_id'] + self.mean_columns].groupby('group_id').mean()
        agg_mode = group_reservations[['group_id'] + self.mode_columns].groupby('group_id').agg(lambda x: x.value_counts().index[0])
        agg_first = group_reservations[['group_id'] + self.first_columns].groupby('group_id').first()
        
        group_reservations = agg_sum.merge(agg_mean, on='group_id', how='inner')
        group_reservations = group_reservations.merge(agg_mode, on='group_id', how='inner')
        group_reservations = group_reservations.merge(agg_first, on='group_id', how='inner')
        group_reservations = group_reservations.reset_index(drop=True)
        
        df = pd.concat([non_group_reservations, group_reservations], ignore_index=True)
        return df
        ########################
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        """
        ########################
        df_avg = df.groupby('room_group_id').mean().reset_index()[['room_group_id', 'night_price']]
        df_avg.rename(columns={'room_group_id': 'room_group_id', 'night_price': 'avg_room_price'}, inplace=True)
        df = df.merge(df_avg, on='room_group_id', how='left')
        df['room_segment'] = df['avg_room_price'].apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
        df.drop(columns='night_price', axis=1, inplace=True)
        return df
        ########################
